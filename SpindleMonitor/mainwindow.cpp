    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->actionactionStart,&QAction::triggered,this,&MainWindow::SlotStart);
    //单张采集
    connect(ui->actionactionStop,&QAction::triggered,this,&MainWindow::SlotStop);

    //判断线程是否存在，并且所有的照片已经处理完成，关闭线程，释放内存
    connect(ui->actionactionStop,&QAction::triggered,this,&MainWindow::closeImgDealThread);
...
//                emit leftRightOriginSignal(g_handle,4);
    ...
    ccfpath = exepath + "/T_Linea_M4096-7um_Default_Default.ccf";
    qDebug() << "ccfpath " << ccfpath;
    QFileInfo file(ccfpath);
    if(!file.isFile()) //文件不存在
    {
        QDateTime current_date_time =QDateTime::currentDateTime();
        QString current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss: ");
        ui->textEditLog->append(current_date + QString("相机配置文件不存在，请查找!"));
    }
    else
    {
    ...
    char IP[128] = "192.168.0.11";

    int ret = ZAux_OpenEth(IP,&g_handle);
    ...
    if(ret == 0)
    {
        ...
        for(int i=0 ;i<2;i++)
        {
            ...
            //设定脉冲模式及逻辑方向（脉冲+方向）
            ZAux_Direct_SetInvertStep(g_handle, i, 2);
